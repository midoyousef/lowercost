...
function searchresult() {
  ...
  httprequest.open("GET", "http://aaaserver-001-site31.ftempurl.com/" + lang + "/api/Search/Search/" + search + "?Index=" + pageNum + "&Size=10");
  httprequest.send();
  httprequest.onreadystatechange = function () {
    if (httprequest.status == 200 && httprequest.readyState == 4) {
      ...
      if (Data.length == 0) {
        document.getElementById("pagesNumber").innerHTML = "<p>No Data with this serach</p>";
      } else {
        displayresult(Data);
...
function displayresult(products) {
  ...
  document.getElementById("searchresult").innerHTML = shopitem;
